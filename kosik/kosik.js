...
window.updateQuantity = function(index, action) {
    if (cart[index]) { // Kontrola, jestli produkt v košíku existuje
        if (action === 'increase') {
            cart[index].quantity = (cart[index].quantity || 0) + 1; // Zvýšení množství
        } else if (action === 'decrease' && cart[index].quantity > 1) {
            cart[index].quantity--; // Snížení množství, ale minimálně na 1
        }
        saveCart(); // Uložení do localStorage
        renderCart(); // Aktualizace zobrazení
    }
};
